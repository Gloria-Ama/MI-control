    ...
    async function chargerDonnees() {
        ...
        console.log("Erreur finances:", err);
    ...
    async function prendrePhoto() {
        ...
        if (!permission.granted) {
            Alert.alert("Permission refusée", "Autorisation d'accès à l'appareil photo requise.");
            return;
        }
    ...
    async function soumettreDemande() {
        if (!montantDemande.trim() || !motif.trim()) {
        Alert.alert("Champs requis", "Le montant et le motif sont obligatoires.");
        return;
        }
        if (isNaN(Number(montantDemande)) || Number(montantDemande) <= 0) {
    ...
    async function soumettreTransaction() {
        if (!montantTransaction.trim() || isNaN(Number(montantTransaction)) || Number(montantTransaction) <= 0) {
        Alert.alert("Montant invalide", "Entrez un montant valide.");
        return;
        }
        setSauvegarde(true);
        ...
        await createTransaction({
            ...
            montant: Number(montantTransaction),
            description: descriptionTransaction.trim(),
            date: dateTransaction,
            communaute_culte: communauteId,
            culte_date: dateTransaction,
        });
    ...
    async function handleRembourser(demande: Demande) {
        await rembourserDemande(demande.id);
        await chargerDonnees();
    }
    ...
    const demandesFiltrees = filtreStatut
        ? demandes.filter(d => d.statut === filtreStatut)
        : demandes;

    const deptNom = departementId
        ? departements.find(d => d.id === departementId)?.nom ?? "Choisir..."
        : "Aucun département";
    ...
    return (
        ...
        >
            ...
            >
                <Text style={[fs.ongletTexte, onglet === o.id && fs.ongletTexteActif]}>
                {o.label}
                </Text>
            </Pressable>
            ))}
        ...
        {onglet === "demandes" && (
            <>
            {/* Filtres — pilules horizontales compactes */}
            <View style={{
                backgroundColor: "#fff",
                ...
                {FILTRES_STATUT.map(f => (
                    ...
                    <Text style={{
                        fontSize: 13,
                        color: filtreStatut === f.valeur ? "#fff" : "#1E293B",
                        fontWeight: filtreStatut === f.valeur ? "700" : "400",
                    }}>
                        {f.label}
                    </Text>
                    </Pressable>
            ...
            {chargement ? (
                ...
                {demandesFiltrees.length === 0 && (
                    <Text style={fs.videTexte}>Aucune demande trouvée.</Text>
                )}
                {demandesFiltrees.map(d => {
                    ...
                    <View key={d.id} style={fs.demandeCard}>
                        ...
                        <Text style={fs.infoLabel}>Département</Text>
                        <Text style={fs.infoValeur}>{d.departement_nom ?? "—"}</Text>
                        </View>
                        <View style={fs.infoRow}>
                        <Text style={fs.infoLabel}>Demandé par</Text>
                        <Text style={fs.infoValeur}>{d.responsable_nom ?? "—"}</Text>
                        </View>
                        <View style={fs.infoRow}>
                        <Text style={fs.infoLabel}>Date</Text>
                        <Text style={fs.infoValeur}>{formatDate(d.date_demande)}</Text>
                        </View>
                        <View style={fs.infoRow}>
                        <Text style={fs.infoLabel}>Motif</Text>
                        <Text style={fs.infoValeur}>{d.motif}</Text>
                        </View>
                        {d.notes_traitement ? (
                        <View style={fs.infoRow}>
                            <Text style={fs.infoLabel}>Notes</Text>
                            <Text style={fs.infoValeur}>{d.notes_traitement}</Text>
                        </View>
                        ) : null}
                        ...
                        <ScrollView horizontal showsHorizontalScrollIndicator={false} style={{ marginTop: 8 }}>
                            ...
                            <Pressable key={i} onPress={() => setImageSelectionnee(uri)}>
                                <Image source={{ uri }} style={fs.piecePreview} />
                            </Pressable>
                            ))}
                        </ScrollView>
                        )}
                        ...
                        <View style={fs.actionsRow}>
                            <Pressable style={fs.btnApprouver} onPress={() => handleApprouver(d)}>
                            <Text style={fs.btnApprouverTexte}>✅ Approuver</Text>
                            </Pressable>
                            <Pressable style={fs.btnRefuser} onPress={() => handleRefuser(d)}>
                            <Text style={fs.btnRefuserTexte}>❌ Refuser</Text>
                            </Pressable>
                        </View>
                        )}
                        {peutApprouver && d.statut === "approuvee" && (
                        <View style={fs.actionsRow}>
                            <Pressable style={fs.btnRembourser} onPress={() => handleRembourser(d)}>
                            <Text style={fs.btnRembourserTexte}>💸 Marquer remboursé</Text>
                            </Pressable>
                        </View>
                        )}
                    </View>
        ...
        {onglet === "nouvelle_demande" && (
            <ScrollView style={fs.formulaire} contentContainerStyle={{ paddingBottom: 100 }}>
            <Text style={fs.formTitre}>Nouvelle demande</Text>

            <Text style={fs.champLabel}>Type</Text>
            <View style={fs.typeRow}>
                <Pressable
                style={[fs.typeBtn, typeDemande === "financement" && fs.typeBtnActif]}
                onPress={() => setTypeDemande("financement")}
                >
                <Text style={[fs.typeBtnTexte, typeDemande === "financement" && fs.typeBtnTexteActif]}>
                    💰 Financement
                </Text>
                </Pressable>
                <Pressable
                style={[fs.typeBtn, typeDemande === "remboursement" && fs.typeBtnActif]}
                onPress={() => setTypeDemande("remboursement")}
                >
                <Text style={[fs.typeBtnTexte, typeDemande === "remboursement" && fs.typeBtnTexteActif]}>
                    🧾 Remboursement
                </Text>
                </Pressable>
            </View>
            ...
            <TextInput
                ...
                onChangeText={setMontantDemande}
            ...
            {deptOuvert && (
                ...
                <Pressable
                    style={fs.deptOption}
                    onPress={() => { setDepartementId(null); setDeptOuvert(false); }}
                >
                    <Text style={[fs.deptOptionTexte, !departementId && fs.deptOptionTexteActif]}>
                    Aucun département
                    </Text>
                ...
                {departements.map((d, i) => {
                    ...
                    <Pressable
                        ...
                        style={[
                        fs.deptOption,
                        sel && fs.deptOptionActif,
                        i === departements.length - 1 && { borderBottomWidth: 0 },
                        ]}
                        onPress={() => { setDepartementId(d.id); setDeptOuvert(false); }}
                    >
            ...
            <TextInput
                style={[fs.champInput, fs.champInputMulti]}
                value={motif}
            ...
            <View style={fs.piecesBox}>
                ...
                <Pressable style={[fs.piecesAjouter, { flex: 1 }]} onPress={prendrePhoto}>
                    <Text style={fs.piecesAjouterTexte}>📷 Prendre une photo</Text>
                </Pressable>
                <Pressable style={[fs.piecesAjouter, { flex: 1 }]} onPress={choisirPhoto}>
                    <Text style={fs.piecesAjouterTexte}>🖼 Galerie</Text>
                </Pressable>
                </View>
            ...
            <Pressable
                style={[fs.btnSoumettre, sauvegarde && { opacity: 0.6 }]}
                onPress={soumettreDemande}
                disabled={sauvegarde}
            >
                {sauvegarde
                ? <ActivityIndicator color="#fff" />
                : <Text style={fs.btnSoumettreTexte}>Soumettre la demande</Text>
                }
            </Pressable>
            </ScrollView>
        )}
        ...
        {onglet === "comptabilite" && (
            ...
            {resume && (
                ...
                <Text style={fs.resumeSolde}>
                    {resume.solde >= 0 ? "+" : ""}{resume.solde.toFixed(2)} $
                </Text>
            ...
            {resume && (
                ...
                {Object.entries(resume.par_type)
                    ...
                    return (
                        ...
                        marginBottom: 6, borderWidth: 0.5, borderColor: "#E2E8F0",
                        }}>
            ...
            <View style={{ paddingHorizontal: 14 }}>
                ...
                transactions.map(t => (
                    <Pressable
                    key={t.id}
                    style={fs.transactionCard}
                    onLongPress={() => handleSupprimerTransaction(t)}
                    >
                    <View style={[
                        fs.transactionIcone,
                        { backgroundColor: t.est_entree ? "#D1FAE5" : "#FEE2E2" }
                    ]}>
        ...
        {onglet === "nouvelle_transaction" && (
            ...
            <View style={{ marginBottom: 16 }}>
                <Text style={{ fontSize: 12, fontWeight: "700", color: "#065F46", marginBottom: 8, textTransform: "uppercase" }}>
                Entrées d'argent
                </Text>
                ...
                <View style={{ flexDirection: "row", gap: 8 }}>
                    ...
                    <Pressable
                        ...
                        borderColor: typeTransaction === t.valeur ? "#065F46" : "#E2E8F0",
                        backgroundColor: typeTransaction === t.valeur ? "#065F46" : "#fff",
                        justifyContent: "center",
                        }}
                    ...
                    >
                        <Text style={{
                        fontSize: 13,
                        color: typeTransaction === t.valeur ? "#fff" : "#1E293B",
                        fontWeight: typeTransaction === t.valeur ? "700" : "400",
                        }}>
                        {t.label}
                        </Text>
                    </Pressable>
                ...
                <Text style={{ fontSize: 12, fontWeight: "700", color: "#991B1B", marginBottom: 8, textTransform: "uppercase" }}>
                Sorties d'argent
                </Text>
                <ScrollView horizontal showsHorizontalScrollIndicator={false}>
                <View style={{ flexDirection: "row", gap: 8 }}>
                    ...
                    <Pressable
                        ...
                        paddingHorizontal: 14, height: 36,
                        borderRadius: 99, borderWidth: 0.5,
                        borderColor: typeTransaction === t.valeur ? "#991B1B" : "#E2E8F0",
                        backgroundColor: typeTransaction === t.valeur ? "#991B1B" : "#fff",
                        justifyContent: "center",
                        }}
                        onPress={() => setTypeTransaction(t.valeur)}
                    >
            ...
            <TextInput
                ...
                onChangeText={setDateTransaction}
            ...
            <Pressable
                style={[fs.btnSoumettre, sauvegarde && { opacity: 0.6 }]}
                onPress={soumettreTransaction}
                disabled={sauvegarde}
            >
                {sauvegarde
                ? <ActivityIndicator color="#fff" />
                : <Text style={fs.btnSoumettreTexte}>Enregistrer</Text>
                }
            </Pressable>
            </ScrollView>
        )}
        ...
        <Modal
            visible={!!imageSelectionnee}
            transparent
            animationType="fade"
            onRequestClose={() => setImageSelectionnee(null)}
        >
            <Pressable
            style={{ flex: 1, backgroundColor: "rgba(0,0,0,0.9)", justifyContent: "center", alignItems: "center" }}
            onPress={() => setImageSelectionnee(null)}
            >
            {imageSelectionnee && (
                <Image
                source={{ uri: imageSelectionnee }}
                style={{ width: "90%", height: "70%", borderRadius: 12 }}
                resizeMode="contain"
                />
            )}
            <Text style={{ color: "rgba(255,255,255,0.6)", marginTop: 16, fontSize: 13 }}>
                Appuyer pour fermer
            </Text>
            </Pressable>
        </Modal>
